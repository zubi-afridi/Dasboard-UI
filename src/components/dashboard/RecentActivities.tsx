...
export function RecentActivities() {
  ...
  const rows = useMemo(() => {
    const filtered = filterRows(ALL_ACTIVITIES, search);
  ...
  return (
    <article className="neo-card rounded-[22px] px-4 py-5 sm:rounded-[26px] sm:px-6 sm:py-6 md:rounded-[26px] md:px-5 md:py-5 xl:rounded-[30px] xl:px-6 xl:py-6">
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-xl font-bold leading-tight text-[#2E78FF] xl:text-[1.375rem]">
          Recent Activities
        </h2>
        <Link
          href="/activities"
          className="neo-link-btn w-fit text-sm font-semibold text-[#2D7CFF]"
        >
          View All →
        </Link>
      </div>
      ...
      {rows.length === 0 ? (
        <p className="rounded-[18px] bg-[#F4F7FB] px-4 py-6 text-center text-sm font-medium text-[#8B9BB4]">
          No activities match "{search.trim()}". Try another search.
        </p>
      ) : (
        <>
          {/* Tablet / desktop: table */}
          <div className="hidden overflow-x-auto md:block">
            <div className="min-w-[480px] xl:min-w-[560px]">
          ...
          >
            {rows.map((row) => (
              <li key={`${row.name}-${row.date}-mobile`}>
